...
J'ai repris les contraintes du sujet et les recommandations ANSSI : RSA au moins 2048 bits, courbes elliptiques au moins 256 bits et SHA-2. Les valeurs techniques restent celles demandées.
...
Sa validité est donc de \textbf{7 jours}. Les premières entrées de \file{crl.txt} montrent surtout le numéro de série et la date de révocation; des \file{X509v3 CRL Reason Code} apparaissent plus loin, mais pas dès le début du fichier. Pour vérifier que la CRL est la bonne, j'ai comparé son \file{Issuer} à \file{C=FR, O=Gandi SAS, CN=GandiCert}, vérifié que l'URL venait bien des points de distribution, puis validé la signature avec le certificat de l'AC. La recherche du numéro de série \file{057E1FDDC48E7A0780643ABEEBCC52A5} ne renvoie aucune correspondance : le certificat n'est pas listé comme révoqué dans cette CRL.
...
Je conserve les contraintes \file{pathLenConstraint} : \file{pathlen:2} pour la racine RSA, \file{pathlen:1} pour la racine EC et \file{pathlen:0} pour les deux subordonnées.
...
J'ai émis le certificat \file{Sub_RSA_CA_1_APV-leaf-crl.crt} dans \file{03-crl-ocsp} avec \file{Sub_RSA_CA_1_APV}.
...
Le certificat \file{04-smime/smime-user.crt} est émis par \file{Sub_RSA_CA_1_APV}. J'ai conservé les extensions suivantes : \file{Basic Constraints: CA:FALSE}, \file{Key Usage: Digital Signature, Key Encipherment}, \file{Extended Key Usage: E-mail Protection} et le SAN email \file{smime-user@apv.local}.
...
\begin{remarkbox}
Le sujet recommande idéalement deux certificats distincts, un pour la signature et un autre pour le chiffrement. J'ai gardé un certificat unique pour simplifier les manipulations.
\end{remarkbox}
...
Je n'ai pas exécuté cette étape dans l'environnement Linux. Les certificats à importer sont la racine et l'intermédiaire correspondant à la chaîne testée. Les magasins Windows visés sont \file{Cert:\LocalMachine\Root} pour la racine et \file{Cert:\LocalMachine\CA} pour l'intermédiaire.
...
La tentative avec le provider \file{pkcs11} a échoué, simplement parce que le module attendu ne portait pas ce nom. J'ai donc retenu le provider OpenSSL 3 \file{pkcs11prov}, avec le module SoftHSM2 configuré.

La partie ADCS doit être exécutée sur Windows Server 2019. Le dossier \file{07-windows-handoff/README.md} documente la préparation, mais je n'ai pas de preuve d'exécution finale.
